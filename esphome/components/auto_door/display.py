...
async def to_code(config):
    ...
    cg.add(var.set_drive_pin(await cg.gpio_pin_expression(config[CONF_DRIVE_PIN])))
    cg.add(var.set_dir_pin(await cg.gpio_pin_expression(config[CONF_DIR_PIN])))
    cg.add(
        var.set_rotsen_pin(await cg.gpio_pin_expression(config[CONF_ROT_SENSOR_PIN]))
    )
    cg.add(
        var.set_engage_pin(await cg.gpio_pin_expression(config[CONF_ENGAGE_MOTOR_PIN]))
    )
    cg.add(var.set_pot_pin(await cg.gpio_pin_expression(config[CONF_POT_PIN])))
    cg.add(var.set_esoff_pin(await cg.gpio_pin_expression(config[CONF_ES_OFF_PIN])))
    cg.add(var.set_eson_pin(await cg.gpio_pin_expression(config[CONF_ES_ON_PIN])))
