...
Servo::Servo() {  // initialize this channel with plausible values, except pin # (we set pin # when attached)
  REFRESH_CPS = 50;
...
int Servo::attach(int pin) { return (this->attach(pin, DEFAULT_uS_LOW, DEFAULT_uS_HIGH)); }
...
int Servo::attach(int pin, int min, int max) {
  ...
  if (pwm.hasPwm(pin)) {
    ...
    if (this->pinNumber <
        0)  // we are attaching to a new or previously detached pin; we need to initialize/reinitialize
    {
      this->ticks = DEFAULT_PULSE_WIDTH_TICKS;
      this->timer_width = DEFAULT_TIMER_WIDTH;
      this->timer_width_ticks = pow(2, this->timer_width);
...
int Servo::readTicks() { return this->ticks; }
