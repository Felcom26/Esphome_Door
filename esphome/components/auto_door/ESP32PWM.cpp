  ...
  if (pwmChannel < 0) {
    for (int i = 0; i < 4; i++) {
      ...
      if (freqAllocated && timerCount[i] < 4) {
        ...
        // Serial.println("Free channel timer "+String(i)+" at freq "+String(freq)+" remaining
        // "+String(4-timerCount[i]));
        ...
        //				if(timerFreqSet[i]>0)
        //					Serial.println("Timer freq mismatch target="+String(freq)+" on timer "+String(i)+" was
        //"+String(timerFreqSet[i])); 				else 					Serial.println("Timer out of channels
        // target="+String(freq)+" on timer
        //"+String(i)+" was "+String(timerCount[i]));
      }
    }
  } else {
    return pwmChannel;
...
#else
  ESP_LOGE(TAG, "ERROR PWM channel unavailable on pin requested! %d PWM available on: 2,4,5,12-19,21-23,25-27,32-33",
           pin);
#endif
}
