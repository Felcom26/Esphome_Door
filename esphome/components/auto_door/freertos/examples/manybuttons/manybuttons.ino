...
void setup(void) {
  ...
  xTaskCreate(vPrintTask, (signed portCHAR *) "vPrintTask", 120, NULL, tskIDLE_PRIORITY, NULL);

  // One task for each button
  int i = sizeof(button_pins);
  while (i--) {
    xTaskCreate(vButtonTask, (signed portCHAR *) "vButtonTask", 70, button_pins + i, tskIDLE_PRIORITY, NULL);
  }
