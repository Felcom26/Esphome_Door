...
#define SOC_ADC_PATT_LEN_MAX \
  (16)  // Two pattern table, each contains 16 items. Each item takes 1 byte. But only support ADC1 using DMA mode
...
#define SOC_I2S_SUPPORTS_PCM2PDM \
  (1)  // Support to write PCM format but output PDM format data with the help of PCM to PDM filter
#define SOC_I2S_SUPPORTS_PDM_RX (1)  // Support to input raw PDM format data
#define SOC_I2S_SUPPORTS_PDM2PCM \
  (1)  // Support to input PDM format but read PCM format data with the help of PDM to PCM filter
...
#define SOC_TOUCH_SAMPLE_CFG_NUM \
  (1U) /*!< The sample configuration number in total, each sampler can be used to sample on one frequency */
...
#define SOC_BT_H2C_ENC_KEY_CTRL_ENH_VSC_SUPPORTED \
  (1) /*!< Support Bluetooth Classic encryption key size configuration through vendor-specific HCI command */
