...
analog_write_channel_t _analog_write_channels[NUM_CHANNELS] = {
    {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8},
    {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8},
    {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}, {-1, 5000, 8}};
...
void analogServo(int8_t pin, uint32_t value) {
  ...
  if (channel < NUM_CHANNELS) {
    ...
    if (_analog_write_channels[channel].frequency != SERVO_FREQUENCY ||
    ...
    if (value < SERVO_MAX_ANGLE + 1) {
      ...
      ledcWrite(channel, map(value, SERVO_MIN_PULSE_WIDTH, SERVO_MAX_PULSE_WIDTH, SERVO_MIN_TICKS,
                             SERVO_MAX_TICKS));  // map ms to ticks
    } else {
      ledcWrite(channel, 0);
    }
  }
